...
# 顶层数据结构
class ItemUserInfoConfig(TypedDict):
    """物品用户信息配置数据"""

    root: _Root


class ItemUserInfoParser(BaseParser[ItemUserInfoConfig]):
    """物品用户信息配置解析器"""
    ...
    def parse(self, data: bytes) -> ItemUserInfoConfig:
        reader = BytesReader(data)
        result: ItemUserInfoConfig = {'root': {'item': []}}
